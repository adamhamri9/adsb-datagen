...
class TestEncodeAltitude:
    def test_returns_an_integer(self):
        result = ADSBMath.encode_altitude(0)

        assert isinstance(result, int)

    def test_output_is_13_bit_unsigned(self):
        for alt in [-1000, 0, 11000, 50100]:
            result = ADSBMath.encode_altitude(alt)
            assert 0 <= result <= 0x1FFF

    def test_is_deterministic(self):
        for alt in [0, 1000, 11000, -500]:
            assert ADSBMath.encode_altitude(alt) == ADSBMath.encode_altitude(alt)
    ...
    def test_output_increases_with_altitude(self):
        ...
        encoded = [ADSBMath.encode_altitude(a) for a in alts]
        assert encoded == sorted(encoded)

    def test_25_foot_resolution(self):
        base = ADSBMath.encode_altitude(0)
        same_bucket = ADSBMath.encode_altitude(24)
        next_bucket = ADSBMath.encode_altitude(25)
